...
class Build(object):
    ...
    def api_representation(self):
        return {
            'id': self._build_id,
            'status': self._status(),
            'artifacts': self._artifacts_archive_file,
            # todo: this should probably be a url, not a file path
    ...
    def prepare(self, subjobs, job_config):
        ...
        if self.project_type is None:
            raise RuntimeError(
                'prepare() was called before generate_project_type() on build {}.'.format(
                    self._build_id))
        ...
        self._timing_file_path = self.project_type.timing_file_path(
            job_config.name)
    ...
    def begin_subjob_executions_on_slave(self, slave):
        ...
        :type slave: Slave
        """
        analytics.record_event(analytics.BUILD_SETUP_FINISH,
                               build_id=self.build_id(),
                               slave_id=slave.id)
        for slave_executor_count in range(slave.num_executors):
            if (self._num_executors_in_use >= self._max_executors or
                slave_executor_count >= self._max_executors_per_slave):
                break
    ...
    def cancel(self):
        ...
        if self._status() in [BuildStatus.FINISHED, BuildStatus.ERROR,
    ...
    def _num_atoms(self):
        if self._status() not in [BuildStatus.BUILDING, BuildStatus.FINISHED]:
            return None
        return sum([len(subjob.atomic_commands())
                    for subjob in self._all_subjobs_by_id.values()])

    @property
    def _subjobs_are_finished(self):
        return self._is_canceled or (self.is_prepared and
                                     self._finished_subjobs.full())
    ...
    def _build_results_dir(self):
        return os.path.join(Configuration['results_directory'],
                            str(self.build_id()), )
