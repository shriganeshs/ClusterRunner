...
class ClusterSlaveApplication(ClusterApplication):
    def __init__(self, cluster_slave):
        ...
        """
        default_params = {'cluster_slave': cluster_slave}
...
class _APIVersionOneHandler(_ClusterSlaveBaseHandler):
    def get(self):
        response = {'slave': self._cluster_slave.api_representation(), }
        self.write(response)
...
class _SubjobHandler(_ClusterSlaveBaseHandler):
    ...
    def post(self, build_id, subjob_id):
        ...
        response = self._cluster_slave.start_working_on_subjob(
            int(build_id), int(subjob_id), subjob_artifact_dir,
            atomic_commands)
        self._write_status(response, status_code=201)

    def get(self, build_id, subjob_id):
        response = {'comment': 'not implemented', }
        self.write(response)
    ...
    def get(self):
        response = {
            'executors': [
                ...
                for executor in self._cluster_slave.executors_by_id.values()
            ]
        }
        self.write(response)
...
class _ExecutorHandler(_ClusterSlaveBaseHandler):
    def get(self, executor_id):
        ...
        response = {'executor': executor.api_representation()}
        self.write(response)
...
class _KillHandler(_ClusterSlaveBaseHandler):
    def post(self):
        self._write_status()
        kill_thread = SafeThread(name='kill-thread',
