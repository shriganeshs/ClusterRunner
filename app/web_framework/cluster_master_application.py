...
class ClusterMasterApplication(ClusterApplication):
    def __init__(self, cluster_master):
        ...
        """
        default_params = {'cluster_master': cluster_master, }
        ...
        root_route = \
            RouteNode(r'/', _RootHandler).add_children([RouteNode(r'v1', _APIVersionOneHandler).add_children([RouteNode(r'version', _VersionHandler), RouteNode(r'build', _BuildsHandler, 'builds').add_children([RouteNode(r'(\d+)', _BuildHandler, 'build').add_children([RouteNode(r'result', _BuildResultRedirectHandler), RouteNode(r'artifacts.tar.gz', _BuildResultHandler), RouteNode(r'subjob', _SubjobsHandler, 'subjobs').add_children([RouteNode(r'(\d+)', _SubjobHandler, 'subjob').add_children([RouteNode(r'atom', _AtomsHandler, 'atoms').add_children([RouteNode(r'(\d+)', _AtomHandler, 'atom')]), RouteNode(r'result', _SubjobResultHandler)])])])]), RouteNode(r'queue', _QueueHandler), RouteNode(r'slave', _SlavesHandler, 'slaves').add_children([RouteNode(r'(\d+)', _SlaveHandler, 'slave')]), RouteNode(r'eventlog', _EventlogHandler)])])
...
class _APIVersionOneHandler(_ClusterMasterBaseHandler):
    def get(self):
        response = {'master': self._cluster_master.api_representation(), }
        self.write(response)
...
class _QueueHandler(_ClusterMasterBaseHandler):
    def get(self):
        response = {
            'queue': [build.api_representation()
                      for build in self._cluster_master.active_builds()]
        }
        self.write(response)
...
class _SubjobsHandler(_ClusterMasterBaseHandler):
    def get(self, build_id):
        ...
        response = {
            'subjobs': [subjob.api_representation()
...
class _SubjobHandler(_ClusterMasterBaseHandler):
    def get(self, build_id, subjob_id):
        ...
        response = {'subjob': subjob.api_representation()}
        self.write(response)
...
class _SubjobResultHandler(_ClusterMasterBaseHandler):
    def post(self, build_id, subjob_id):
        ...
        slave_executor_id = self.decoded_body.get('metric_data', {}).get(
            'executor_id')
        analytics.record_event(analytics.MASTER_RECEIVED_RESULT,
                               executor_id=slave_executor_id,
...
class _AtomsHandler(_ClusterMasterBaseHandler):
    def get(self, build_id, subjob_id):
        build = self._cluster_master.get_build(int(build_id))
        subjob = build.subjob(int(subjob_id))
        response = {'atoms': subjob.get_atoms(), }
        self.write(response)
...
class _AtomHandler(_ClusterMasterBaseHandler):
    def get(self, build_id, subjob_id, atom_id):
        ...
        response = {'atom': atoms[int(atom_id)], }
        self.write(response)
...
class _BuildsHandler(_ClusterMasterBaseHandler):
    ...
    def post(self):
        ...
        success, response = self._cluster_master.handle_request_for_new_build(
            build_params)
    ...
    def get(self):
        response = {
            'builds': [build.api_representation()
                       for build in self._cluster_master.builds()]
        }
        self.write(response)
...
class _BuildHandler(_ClusterMasterBaseHandler):
    ...
    def put(self, build_id):
        ...
        success, response = self._cluster_master.handle_request_to_update_build(
    ...
    def get(self, build_id):
        response = {
            'build': self._cluster_master.get_build(
                int(build_id)).api_representation(),
        }
        self.write(response)
...
class _BuildResultHandler(tornado.web.StaticFileHandler):
    ...
    def initialize(self, route_node=None, cluster_master=None):
        ...
        super().initialize(
            path=None
        )  # we will not set the root path until the get() method is called

    def get(self, build_id, path=None):
        artifact_file_path = self._cluster_master.get_path_for_build_results_archive(
            int(build_id))
        self.root, artifact_filename = os.path.split(artifact_file_path)
        self.set_header('Content-Type', 'application/octet-stream'
                       )  # this should be downloaded as a binary file
...
class _SlavesHandler(_ClusterMasterBaseHandler):
    def post(self):
        ...
        response = self._cluster_master.connect_new_slave(slave_url,
                                                          num_executors)
    ...
    def get(self):
        response = {
            'slaves': [
                ...
                for slave in self._cluster_master.all_slaves_by_id().values()
            ]
        }
        self.write(response)
