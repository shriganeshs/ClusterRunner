...
class DeployTarget(object):
    ...
    def deploy_binary(self, source_tar, dest_dir):
        ...
        self._shell_client.exec_command(
            'rm -rf {0}; mkdir -p {0}'.format(dest_dir),
            error_on_failure=True)
        self._shell_client.copy(source_tar,
                                '{}/clusterrunner.tgz'.format(parent_dest_dir))
        self._shell_client.exec_command(
            command='tar zxvf {}/clusterrunner.tgz -C {}'.format(
                parent_dest_dir, dest_dir),
            error_on_failure=True)
    ...
    def deploy_conf(self, source_path, dest_path):
        ...
        if not os.path.exists(source_path):
            raise RuntimeError(
                'Expected configuration file to exist in {}, but does not.'.format(
                    source_path))

        self._shell_client.copy(source_path, dest_path)
        # Must set permissions of conf to '600' for security purposes.
        self._shell_client.exec_command('chmod 600 {}'.format(dest_path),
                                        error_on_failure=True)
