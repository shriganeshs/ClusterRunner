...
class TestTimeBasedAtomGrouper(BaseUnitTestCase):
    ...
    def test_groupings_data_set_1(self):
        new_atoms = ['atom_1', 'atom_2', 'atom_3', 'atom_4', 'atom_5',
                     'atom_6', 'atom_7', 'atom_8', 'atom_9', 'atom_10']
        old_atoms_with_times = dict({
            'atom_1': 1.0,
            'atom_2': 10.0,
            'atom_3': 11.0,
        ...
        expected_groupings = [['atom_2', 'atom_3', 'atom_10'],
                              ['atom_4', 'atom_5', 'atom_7',
                               'atom_9'], ['atom_8'], ['atom_6'], ['atom_1']]

        atom_grouper = TimeBasedAtomGrouper(new_atoms, 2, old_atoms_with_times,
                                            'some_project_directory')
    ...
    def test_groupings_data_set_2(self):
        ...
        expected_groupings = [['atom_1'], ['atom_2'], ['atom_3', 'atom_4',
                                                       'atom_5']]
    ...
    def test_groupings_data_set_4(self):
        ...
        atom_grouper = TimeBasedAtomGrouper(new_atoms, 5, old_atoms_with_times,
                                            'some_project_directory')
        subjobs = atom_grouper.groupings()
    ...
    def test_groupings_maintains_project_directory_in_returned_atoms(self):
        new_atoms = ['/var/clusterrunner/repos/scm/atom_1',
        ...
        old_atoms_with_times = dict(
            {'atom_1': 100.0,
             'atom_2': 100.0,
             'atom_3': 100.0})
        expected_groupings = [['/var/clusterrunner/repos/scm/atom_1'],
                              ['/var/clusterrunner/repos/scm/atom_2'],
                              ['/var/clusterrunner/repos/scm/atom_3']]
        atom_grouper = TimeBasedAtomGrouper(new_atoms, 3, old_atoms_with_times,
                                            '/var/clusterrunner/repos')
        subjobs = atom_grouper.groupings()

        self._assert_groupings_equal(expected_groupings, subjobs)

    def _assert_coalesced_contents(self, coalesced_atoms_with_historic_times,
                                   expected_atom_time_values):
        """
        ...
        """
        self.assertEquals(len(coalesced_atoms_with_historic_times),
                          len(expected_atom_time_values))

        for atom_time_pair in coalesced_atoms_with_historic_times:
            atom = atom_time_pair[0]
            time = atom_time_pair[1]
            self.assertTrue(atom_time_pair[0] in expected_atom_time_values,
                            'No entry for Atom: ' + atom + ' found')

            if atom in expected_atom_time_values:
                self.assertEquals(time, expected_atom_time_values[atom],
                                  'Incorrect time for atom ' + atom)
    ...
    def _assert_groupings_equal(self, expected, actual):
        ...
        self.assertEquals(len(expected), len(actual),
                          'Incorrect number of subjobs created!')
