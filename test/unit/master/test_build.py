...
class TestBuild(BaseUnitTestCase):
    ...
    def setUp(self):
        super().setUp()
        mock_util = self.patch(
            'app.master.build.app.util'
        )  # stub out util since these often interact with the fs
    ...
    def test_build_doesnt_use_more_than_max_executors(self):
        ...
        mock_slaves = [self._create_mock_slave(num_executors=5)
                       for _ in range(3)]
        ...
        build.prepare(subjobs, self._create_job_config(
            max_executors=expected_num_executors))
        ...
        self.assertEqual(
            build.execute_next_subjob_or_teardown_slave.call_count,
    ...
    def test_complete_subjob_writes_and_extracts_payload_to_correct_directory(
        self
    ):
        ...
        payload = {
            ...
            'body': 'Heroes in a half shell.'
        }
        build.complete_subjob(subjob.subjob_id(), payload=payload)

        self.mock_fs.write_file.assert_called_once_with(
            'Heroes in a half shell.', '/tmp/results/1/turtles.txt')
        self.mock_fs.extract_tar.assert_called_once_with(
            '/tmp/results/1/turtles.txt',
            delete=True)
    ...
    def test_need_more_slaves_returns_false_if_max_processes_is_reached(self):
        ...
        build.allocate_slave(mock_slave)
        self.assertFalse(build.needs_more_slaves(
    ...
    def test_need_more_slaves_returns_true_if_max_processes_is_not_reached(self
        ...
        build.allocate_slave(mock_slave)
        self.assertTrue(build.needs_more_slaves(
    ...
    def test_cancel_depletes_queue_and_sets_canceled(self):
        ...
        self.assertTrue(build._unstarted_subjobs.empty(),
                        "Build's unstarted subjobs should've been depleted")
    ...
    def test_validate_update_params_for_cancelling_build(self):
        ...
        success, response = build.validate_update_params(
            {'status': 'canceled'})
    ...
    def test_validate_update_params_rejects_bad_params(self):
        ...
        self.assertEqual({
            'error':
            "Value (foo) is not in list of allowed values (['canceled']) for status"
        }, response, "Error response not expected")
    ...
    def test_validate_update_params_rejects_bad_keys(self):
        ...
        self.assertFalse(success, "Bad status update reported success")
        self.assertEqual(
            {'error': "Key (badkey) is not in list of allowed keys (status)"},
            response, "Error response not expected")
    ...
    def _create_subjobs(self, count=3, build_id=0):
        return [Subjob(build_id=build_id,
                       subjob_id=i,
                       project_type=None,
                       job_config=None,
