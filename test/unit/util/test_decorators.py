...
class TestDecorators(BaseUnitTestCase):
    def test_retry_on_exception_exponential_backoff_doesnt_sleep_if_no_exception(
        self
    ):
    ...
    def test_retry_on_exception_exponential_backoff_doesnt_retry_if_incorrect_exception_type(
        self
    ):
        sleep_patch = self.patch('app.util.decorators.time.sleep')
    ...
    def test_retry_on_exception_exponential_backoff_retries_with_correct_sleep_durations(
        self
    ):
        sleep_patch = self.patch('app.util.decorators.time.sleep')
        ...
        sleep_patch.assert_has_calls([call(1), call(3), call(9), call(27)],
                                     any_order=False)

    def test_retry_on_exception_exponential_backoff_raise_error_if_initial_delay_is_not_positive(
        self
    ):
        with self.assertRaises(RuntimeError):
            self.dummy_method_noop_with_initial_delay_zero()

    def test_retry_on_exception_exponential_backoff_raise_error_if_exponential_factor_is_less_than_one(
        self
    ):
        with self.assertRaises(RuntimeError):
