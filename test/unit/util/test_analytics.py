...
{"__id__": 3, "__tag__": "NETWORK_REQUEST_RECEIVED", "__timestamp__": 1410902729.896991}
    ...
    def test_get_events_returns_all_events_with_no_params_specified(self):
        ...
        self.patch('app.util.analytics._eventlog_file',
                   new='my_fake_eventlog.log')

        events = analytics.get_events()

        returned_event_ids = [event['__id__'] for event in events]
        self.assertListEqual(returned_event_ids, [
            1, 2, 3
    ...
    def test_get_events_raises_exception_if_both_timestamp_and_id_are_specified(
        self
    ):
        self.patch('app.util.analytics._eventlog_file',
                   new='my_fake_eventlog.log')
        ...
        with self.assertRaises(
            ValueError,
            msg=
            'get_events() should raise if both timestamp and id are specified.'):
    ...
    def test_get_events_returns_none_when_no_initialization_performed(self):
        ...
        self.assertEqual(
            events, None,
            'get_events() should return None if no initialization was done.')
