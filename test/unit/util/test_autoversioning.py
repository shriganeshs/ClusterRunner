...
class TestAutoversioning(BaseUnitTestCase):
    def setUp(self):
        super().setUp()
        self.unpatch('app.util.autoversioning.get_version'
                    )  # patched in BaseUnitTestCase for all other tests
        self.patch('app.util.autoversioning._MAJOR_MINOR_VERSION', new='1.0')
        self.check_output_mock = self.patch(
            'app.util.autoversioning.subprocess.check_output')
    ...
    def test_get_version_returns_frozen_version_when_run_from_frozen_package(
        ...
        self.assertFalse(
            self.check_output_mock.called,
            'No subprocess call should be necessary to get frozen version.')
        self.assertEqual(
            actual_version, '1.2.3',
            'get_version() should return what is written in package_version.py.')

    def test_write_package_version_file_writes_a_valid_python_file(self):
        def fake_write_file(file_contents, _):
            vars_set_in_file = {}
            exec(file_contents, {}, vars_set_in_file
                )  # this will raise if file_contents is not valid python code
            self.assertEqual(vars_set_in_file.get('version'), '1.2.3',
                             'The file written should be Python code that '
                             'sets a "version" variable.')

        self.patch('app.util.autoversioning.os')
        self.patch(
            'app.util.autoversioning.fs').write_file.side_effect = fake_write_file
    ...
    def test_write_package_version_file_backs_up_original_file_before_writing(
        ...
        expected_rename_call = call.os.rename(AnythingOfType(str),
                                              AnythingOfType(str))
        expected_write_file_call = call.fs.write_file(AnythingOfType(str),
                                                      AnythingOfType(str))
        self.assertLess(
            parent_mock.method_calls.index(expected_rename_call),
    ...
    def test_calculate_source_version_caches_computed_version(self):
        ...
        self.assertEqual(
            first_return_val, second_return_val,
    ...
    @genty_dataset(
        ...
        head_commit_is_on_branch_with_changed_files=(False, True,
                                                     '1.0.4-commit2-mod'), )
    def test_calculated_source_version_is_correct_when(
        self, commit_is_on_trunk, has_changed_files, expected_version
    ):
    ...
    def _mock_git_commands_output(self,
                                  commit_is_on_trunk=True,
                                  index_has_changed_files=False):
        diff_index_side_effect = subprocess.CalledProcessError(
            1, 'fake') if index_has_changed_files else b'\n'
        ...
        check_output_side_effects_map = {
            ...
            ('rev-list', '--first-parent', 'commit1^..commit3'
            ...
            ('rev-parse', '--verify',
             'origin/master'): current_origin_master_commit,
        }

        def fake_check_output(cmd_args, *args, **kwargs):
            self.assertEqual(
                cmd_args[0], 'git',
                'All commands during autoversioning are expected to be git commands.')
            return_value = check_output_side_effects_map.get(
                tuple(cmd_args[1:]))
            self.assertIsNotNone(
                return_value,
                'Call to check_output with arguments {} should have a mock side effect '
