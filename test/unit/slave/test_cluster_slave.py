...
class TestClusterSlave(BaseUnitTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.mock_network = self.patch(
            'app.slave.cluster_slave.Network').return_value
        self.patch('app.util.fs.compress_directories')

    @genty_dataset(current_build_id_not_set=(None, ),
                   current_build_id_not_matching=(200, ), )
    def test_start_working_on_subjob_called_with_incorrect_build_id_will_raise(
        self, slave_current_build_id
    ):
        ...
        with self.assertRaises(
            BadRequestError,
            msg=
            'Start subjob should raise error if incorrect build_id specified.'):
            slave.start_working_on_subjob(incorrect_build_id, 1, '~/test',
                                          ['ls'])

    @genty_dataset(current_build_id_not_set=(None, ),
                   current_build_id_not_matching=(200, ), )
    def test_teardown_called_with_incorrect_build_id_will_raise(
        self, slave_current_build_id
    ):
        slave = self._create_cluster_slave()
        slave._current_build_id = slave_current_build_id
        ...
        with self.assertRaises(
            BadRequestError,
            msg='Teardown should raise error if incorrect build_id specified.'):
            slave.teardown_build(incorrect_build_id)

    @genty_dataset(responsive_master=(True, ), unresponsive_master=(False, ), )
    def test_disconnect_request_sent_if_and_only_if_master_is_responsive(
        ...
        expected_network_calls = [call.post(slave_creation_url,
                                            data=ANY),
    ...
    def test_signal_shutdown_process_disconnects_from_master_before_killing_executors(
        ...
        disconnect_api_url = 'http://{}/v1/slave/1'.format(
            self._FAKE_MASTER_URL)
        mock_executor = self.patch(
            'app.slave.cluster_slave.SubjobExecutor').return_value

        parent_mock = MagicMock(
        )  # create a parent mock so we can assert on the order of child mock calls.
        ...
        expected_disconnect_call = call.mock_network.put_with_digest(
            disconnect_api_url,
            request_params=ANY,
            secret=ANY,
            error_on_failure=ANY)
        ...
        self.assertEqual(
            1, parent_mock.method_calls.count(expected_disconnect_call),
            'Graceful shutdown should cause the slave to make a disconnect call to the master.')
        self.assertEqual(
            3, parent_mock.method_calls.count(expected_kill_executor_call),
            'Graceful shutdown should cause the slave to kill all its executors.')
        self.assertLess(
            parent_mock.method_calls.index(expected_disconnect_call),
    ...
    def test_shutting_down_before_connecting_to_master_does_not_raise_exception(
        self
    ):
    ...
    def test_shutting_down_after_running_a_build_does_not_raise_exception(self
        ...
        self.patch('app.slave.cluster_slave.open',
                   new=mock_open(read_data=''),
                   create=True)
        slave = self._create_cluster_slave()
        expected_results_api_url = 'http://{}/v1/build/123/subjob/321/result'.format(
            self._FAKE_MASTER_URL)
        expected_idle_api_url = 'http://{}/v1/slave/1'.format(
            self._FAKE_MASTER_URL)
        subjob_done_event, teardown_done_event = self._mock_network_post_and_put(
            expected_results_api_url, expected_idle_api_url)
        slave.connect_to_master(self._FAKE_MASTER_URL)
        slave.setup_build(build_id=123,
                          project_type_params={'type': 'Fake'},
                          build_executor_start_index=0)
        slave.start_working_on_subjob(build_id=123,
                                      subjob_id=321,
                                      subjob_artifact_dir='',
                                      atomic_commands=[])
        # The timeout for this wait() is arbitrary, but it should be generous so the test isn't flaky on slow machines.
        self.assertTrue(
            subjob_done_event.wait(timeout=5),
            'Subjob execution code under test should post to expected '
            'results url very quickly.')
        slave.teardown_build(123)
        self.assertTrue(
            teardown_done_event.wait(timeout=5),
            'Teardown code under test should post to expected idle '
            'url very quickly.')
        self.trigger_graceful_app_shutdown(
        )  # Triggering shutdown should not raise an exception.

    def _mock_network_post_and_put(self, expected_results_api_url,
        ...
        def fake_network_post_and_put(url, *args, **kwargs):
            if url == expected_results_api_url:
                subjob_done_event.set(
                )  # Consider subjob finished once code posts to results url.
    ...
    @genty_dataset(successful_setup=(True, SlaveState.SETUP_COMPLETED),
                   failed_setup=(False, SlaveState.SETUP_FAILED), )
    def test_setup_should_send_correct_state_update_to_master(
        self, is_setup_successful, expected_slave_state
    ):
        expected_slave_data_url = 'http://{}/v1/slave/1'.format(
            self._FAKE_MASTER_URL)
        slave = self._create_cluster_slave()
        slave.connect_to_master(self._FAKE_MASTER_URL)
        ...
        slave._async_setup_build(executors=[],
                                 project_type_params={},
                                 build_executor_start_index=0)

        self.mock_network.put_with_digest.assert_called_once_with(
            expected_slave_data_url,
            request_params={'slave': {'state': expected_slave_state}},
            secret=ANY,
    ...
    def test_setup_build_sets_base_executor_index(self):
        ...
        slave.setup_build(build_id=123,
                          project_type_params={'type': 'Fake'},
                          build_executor_start_index=8)
        self.assertEqual(8, slave._base_executor_index,
                         'Build setup should set _base_executor_index')
    ...
    def test_execute_subjob_passes_base_executor_index_to_executor(self):
        ...
        with patch.object(builtins, 'open', mock_open(read_data='asdf')):
            slave._execute_subjob(build_id=1,
                                  subjob_id=2,
                                  executor=executor,
                                  subjob_artifact_dir='',
                                  atomic_commands=[])
