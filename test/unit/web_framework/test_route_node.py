...
class TestRouteNode(BaseUnitTestCase):
    def get_nested_route_tree(self):
        root_route = \
            node.RouteNode(r'/', _ExampleHandler).add_children([node.RouteNode(r'widget', _ExampleHandler, 'widgets').add_children([node.RouteNode(r'(\d+)', _ExampleHandler, 'widget').add_children([node.RouteNode(r'start', _ExampleHandler), node.RouteNode(r'end', _ExampleHandler)])])])
        return root_route
    ...
    def test_nested_route_should_generate_multipart_regex(self):
        ...
        self.assertEqual('/widget/(\d+)/start/?', inner_route.regex(),
                         'Generated regex does not match expectation.')

    def test_nested_route_should_generate_multipart_template(self):
        root_route = self.get_nested_route_tree()
        inner_route = root_route.children[0].children[0].children[0]
        self.assertEqual(
            '/widget/[widget_id]/start', inner_route.route_template(),
    ...
    def test_ancestors_should_return_parents_recursively(self):
        ...
        self.assertEqual(
            ['[widget_id]', 'widget', '/'], ancestor_names,
    ...
    def test_descendants_should_return_all_children_recursively(self):
        ...
        self.assertEqual(
            ['widget', '[widget_id]', 'start', 'end'], descendant_names,
