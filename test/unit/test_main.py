...
class TestMain(BaseUnitTestCase):
    ...
    def setUp(self):
        ...
        self.mock_tornado = self.patch(
            'app.subcommands.service_subcommand.tornado')
        self.mock_ClusterMaster = self.patch(
            'app.subcommands.master_subcommand.ClusterMaster')
        self.mock_ClusterSlave = self.patch(
            'app.subcommands.slave_subcommand.ClusterSlave')
        self.mock_ClusterMasterApplication = self.patch(
            'app.subcommands.master_subcommand.ClusterMasterApplication')
        self.mock_ClusterSlaveApplication = self.patch(
            'app.subcommands.slave_subcommand.ClusterSlaveApplication')
        self.mock_BuildRunner = self.patch(
            'app.subcommands.build_subcommand.BuildRunner')
        self.mock_ServiceRunner = self.patch(
        ...
        self.patch(
            'app.util.conf.base_config_loader.platform').node.return_value = self._HOSTNAME
        self.patch('app.subcommands.master_subcommand.analytics.initialize')
        self.patch(
            'argparse._sys.stderr'
        )  # Hack to prevent argparse from printing output during tests.
        ...
        self.start_force_kill_countdown_patcher = patch(
            'main._start_app_force_kill_countdown')
    ...
    @genty_dataset(
        only_required_args_passed=(['imaginary', '--hero-name', 'bilbo'],
                                   {'type': 'imaginary',
                                    'hero_name': 'bilbo'}),
        required_and_optional_args_passed=([
            ...
            'hero_name': 'bilbo',
            'party_size': '12'}),
        one_remote_file=(['--remote-file', 'hello', 'fake_url', 'imaginary',
                          '--hero-name', 'drizzt'], {
                              'type': 'imaginary',
                              'hero_name': 'drizzt',
                              'remote_files': {'hello': 'fake_url'}
                          }),
        multiple_remote_files=
        (['--remote-file', 'hello', 'hello_url', '--remote-file', 'goodbye',
          'goodbye_url', 'imaginary', '--hero-name', 'drizzt'], {
              'type': 'imaginary',
              'hero_name': 'drizzt',
              'remote_files': {'hello': 'hello_url',
                               'goodbye': 'goodbye_url'}
          }))
    def test_valid_args_for_build_will_correctly_instantiate_build_runner(
        ...
        self.patch('main.util.project_type_subclasses_by_name').return_value = {
            # mock out project_type subclasses
    ...
    def test_main_exits_with_nonzero_exit_code_if_build_runner_fails(self):
        ...
        self.mock_BuildRunner.assert_called_once_with(
            master_url='smaug:1',
            secret='mellon1234',
            request_params={'type': 'middleearth',
                            'some_other_param': '999'})
    ...
    def test_all_project_type_command_line_args_should_be_documented_with_help_text(
        self
    ):
        ...
        for add_argument_args, add_argument_kwargs in add_argument_calls:
            ...
            self.assertTrue(
                has_argument_help_text,
                'All arguments (including "{}") should have help text specified.'.format(
                    argument_name))

    @genty_dataset(['-V'], ['--version'], ['master'], ['slave', '-p', '12345'],
                   ['build', '--master-url', 'shire.middle-earth.org'])
    def test_parse_args_accepts_valid_arguments(self, valid_arg_set):
        try:
            main._parse_args(valid_arg_set
    ...
    def test_start_app_force_kill_countdown_sends_self_sigkill_after_delay(self
        ...
        self.start_force_kill_countdown_patcher.stop(
        )  # unpatch this method so we can test it
        pid_of_self = 12345
        sleep_duration = 15
        os_kill_args = None
        ...
        self.assertTrue(os_kill_called_event.wait(timeout=5),
                        'os.kill() should be called within a few seconds.')

        mock_time.sleep.assert_called_once_with(sleep_duration)
        self.assertEqual(
            os_kill_args, (pid_of_self, signal.SIGKILL),
    ...
    def mock_configuration_values(self, custom_dictionary, default_value=None):
        mock_configuration = self.patch(
            'app.util.conf.configuration.Configuration')
