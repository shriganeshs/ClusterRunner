...
class TestStopSubcommand(BaseUnitTestCase):
    ...
    def test_stop_subcommand_doesnt_kill_pid_if_pid_file_exists_but_pid_doesnt_exist(
        self
    ):
    ...
    def test_stop_subcommand_doesnt_kill_pid_if_pid_file_and_pid_exist_but_command_isnt_whitelisted(
        ...
        mock_psutil_process.cmdline.return_value = ['python',
                                                    './SOME_OTHER_main.py',
                                                    'other_master']
    ...
    def setup_main_and_child_proc(self, main_proc_pid,
                                  main_is_running_threshold, child_pid,
        ...
        self.patch('psutil.Process').return_value = self.create_process_mock(
            ...
            children=[self.create_process_mock(pid=child_pid,
                                               is_running_threshold=
                                               child_is_running_threshold)])

    def create_process_mock(self, pid,
                            is_running_threshold=None,
                            cmdline=None,
                            children=None):
    ...
    def test_stop_subcommand_kills_main_proc_with_sigkill_if_still_running_after_sigterm(
        self
    ):
        main_proc_pid = 9999
        ...
        self.setup_main_and_child_proc(main_proc_pid, main_threshold,
                                       child_proc_pid, child_threshold)
