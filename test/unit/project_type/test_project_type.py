...
class TestProjectType(BaseUnitTestCase):
    def setUp(self):
        ...
        self.mock_popen = self.patch(
            'app.project_type.project_type.Popen').return_value
        self.mock_killpg = self.patch('os.killpg')
        self.mock_temporary_files = []
        self.mock_TemporaryFile = self.patch(
            'app.project_type.project_type.TemporaryFile',
    ...
    def test_required_constructor_args_are_correctly_detected_without_defaults(
        self
    ):
    ...
    def test_required_constructor_args_are_correctly_detected_with_defaults(
        self
    ):
    ...
    @genty_dataset(
        ...
        arg_with_multiline_param_doc=('wind', 'doc for wind param...', True,
                                      None),
    ...
    def test_constructor_args_info_returns_expected_data(
        self, arg_name, expected_help_string, expected_required_flag,
    ...
    def test_command_exiting_normally_will_break_out_of_command_execution_wait_loop(
        ...
        self.mock_popen.wait.side_effect = [timeout_exc, timeout_exc,
    ...
    def test_timing_out_will_break_out_of_command_execution_wait_loop_and_kill_subprocesses(
        ...
        self.assertEqual(self.mock_killpg.call_count, 1,
    ...
    def test_exception_raised_while_waiting_for_termination_adds_error_message_to_output(
        ...
        exception_message = 'Something terribly horrible just happened!'
        self._simulate_hanging_popen_process(
            fake_returncode=fake_failing_return_code,
            wait_exception=ValueError(exception_message))
    ...
    def test_failing_exit_code_is_injected_when_no_return_code_available(self):
        ...
        self.assertIsInstance(actual_return_code, int,
                              'Returned exit code should always be an int.')
        self.assertNotEqual(
            actual_return_code, 0,
            'Returned exit code should be failing (non-zero) when subprocess '
            'returncode is not available.')

    @genty_dataset(with_specified_timeout=(30, ), with_no_timeout=(None, ), )
    ...
    def _simulate_hanging_popen_process(self,
                                        fake_returncode=0,
                                        wait_exception=None):
        """
        ...
        def fake_wait(timeout=None):
            ...
            self.fail(
                'Popen.wait() should not be called without a timeout before os.killpg has been called.')
