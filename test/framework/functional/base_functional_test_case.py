...
class BaseFunctionalTestCase(TestCase):
    ...
    def tearDown(self):
        ...
        for service in services:
            self.assertEqual(
                service.return_code, 0,
                'Service running on url: {} should exit with code 0, but exited '
    ...
    def assert_build_artifact_contents_match_expected(
        self, build_id, expected_build_artifact_contents
    ):
        """
        ...
        """
        build_artifacts_dir_path = os.path.join(
            self.test_app_base_dir.name, 'results', 'master', str(build_id))
