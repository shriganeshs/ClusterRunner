...
def _parse_args(args):
    ...
    parser.add_argument(
        '-V', '--version',
        action='version',
        version='ClusterRunner ' + autoversioning.get_version())

    subparsers = parser.add_subparsers(
        title='Commands',
        description=
        'See "{} <command> --help" for more info on a specific command.'.format(
            sys.argv[0]),
        dest='subcommand', )
    ...
    build_parser.add_argument('-j', '--job-name',
    ...
    parsed_args = vars(parser.parse_args(args)
                      )  # vars() converts the namespace to a dict
    return parsed_args
...
def _add_project_type_subparsers(build_parser):
    ...
    for project_type_name, project_type_class in project_types.items():
        ...
        env_args_info = project_type_class.constructor_arguments_info(
            blacklist=help_argument_blacklist)
        for arg_name, arg_info in env_args_info.items():
            fixed_arg_name = arg_name.replace('_', '-')
            if isinstance(arg_info.default, bool):
                project_type_parser.add_argument('--' + fixed_arg_name,
                                                 dest=arg_name,
                                                 action='store_true',
                                                 help=arg_info.help,
                                                 default=argparse.SUPPRESS)
                project_type_parser.add_argument('--no-' + fixed_arg_name,
                                                 dest=arg_name,
                                                 action='store_false',
                                                 help=arg_info.help,
                                                 default=argparse.SUPPRESS)
                ...
                project_type_parser.add_argument(
                    '--' + fixed_arg_name,
                    # example: constructor arg "job_name" --> cmd line arg "--job-name"
                    help=arg_info.help,
                    required=arg_info.required,
                    default=argparse.SUPPRESS,
                    # don't add argument to parsed_args unless a value was explicitly specified
                )
...
def _start_app_force_kill_countdown(seconds):
    ...
    threading.Thread(target=log_app_debug_info_and_force_kill_after_delay,
                     name='SuicideThread',
                     daemon=True).start()
